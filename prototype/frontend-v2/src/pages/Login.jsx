import { Context } from '../utils/Store'
import { useEffect, useContext, useState } from 'react';
import { Redirect } from 'react-router';
import Web3 from "web3";
import Web3Modal from "web3modal";
import WalletConnectProvider from "@walletconnect/web3-provider";
import * as CD from '../utils/ContractData'

let provider = '';
let web3 = '';
let account = '';

const providerOptions = {
    // Example with injected providers
    injected: {
        display: {
            logo: "data:image/svg+xml;base64,",
            name: "Browser",
            description: "Verbinde dich mit deinem Browser Wallet"
        },
        package: null
    },
    // Example with WalletConnect provider
    walletconnect: {
        display: {
            logo: "data:image/svg+xml;base64,",
            name: "Mobil",
            description: "Verbinde dich mit deinem mobilen Wallet"
        },
        package: WalletConnectProvider,
        options: {
            infuraId: "5aea93a9924a4366aeea328d987a8f5b" // required
        }
    }
};
const web3Modal = new Web3Modal({
    network: "rinkeby", // optional
    providerOptions // required
});

const connect = async () => {
    try {
        provider = await web3Modal.connect();
        web3 = new Web3(provider);
    } catch {
        alert("Error 1001: Could not connect to web3 provider.");
        window.location.reload();
    }

    await web3.eth.getAccounts(async (error, accounts) => {
        if (accounts.length > 0) {
            account = accounts[0];
        } else {
            alert("Error 1002: No accounts found.");
            window.location.reload();
        }
    });
}

const init = async (dispatch, setInit) => {
    let moki = await new web3.eth.Contract(CD.MOKI_ABI, CD.MOKI);
    let vote = await new web3.eth.Contract(CD.VOTE_ABI, CD.VOTE);
    let whitelist = await new web3.eth.Contract(CD.WHITELIST_ABI, CD.WHITELIST);
    let userdata = await new web3.eth.Contract(CD.USERDATA_ABI, CD.USERDATA);

    await dispatch({ type: 'SET_WEB3', payload: await web3 });
    await dispatch({ type: 'SET_PROVIDER', payload: await provider });
    await dispatch({ type: 'SET_ACCOUNT', payload: await account });
    await dispatch({ type: 'SET_MOKI', payload: moki});
    await dispatch({ type: 'SET_VOTE', payload: vote });
    await dispatch({ type: 'SET_WHITELIST', payload: whitelist });
    await dispatch({ type: 'SET_MANAGER', payload: await new web3.eth.Contract(CD.MANAGER_ABI, CD.MANAGER) });
    await dispatch({ type: 'SET_USERDATA', payload: userdata });
    await dispatch({ type: 'SET_STORE', payload: await new web3.eth.Contract(CD.STORE_ABI, CD.STORE) });
    await dispatch({ type: 'SET_GROUP', payload: await new web3.eth.Contract(CD.DEPLOYGROUP_ABI, CD.DEPLOYGROUP) });
    await dispatch({ type: 'SET_PROJECT', payload: await new web3.eth.Contract(CD.DEPLOYPROJECT_ABI, CD.DEPLOYPROJECT) });
   
    await dispatch({ type: 'SET_USERNAME', payload: await userdata.methods.getName(account).call()});
    await dispatch({ type: 'SET_MOKIAMOUNT', payload: await moki.methods.balanceOf(account).call()});
    await dispatch({ type: 'SET_VOTEAMOUNT', payload: await vote.methods.balanceOf(account).call()});
    await dispatch({ type: 'SET_ISUSER', payload: await whitelist.methods.isUser(account).call()});
    await dispatch({ type: 'SET_ISACCESSHUB', payload: await whitelist.methods.isAccessHub(account).call()});
    await dispatch({ type: 'SET_ISADMIN', payload: await whitelist.methods.isAdmin(account).call()});
    
   
    await setInit(true);
}

const finished = async (dispatch) => {
    dispatch({type: 'SET_INIT', payload: true});
    return;
}

function Login() {
    const [state, dispatch] = useContext(Context);
    const [isInit, setInit] = useState(false);

    useEffect(async () => {
        await connect();
        await init(dispatch, setInit);
        await finished(dispatch);
    },[]);

    if(isInit && state.account !== null && !state.isUser) {
        return <Redirect push to="/register" />
    } else if(isInit && state.account !== null && state.username === "") {
        return <Redirect push to="/addUsername" />
    } else if(isInit) {
        return <Redirect push to="/" />
    }

    return (
        <div className="page login">
            Bitte mit deinem Wallet verbinden :)
        </div>
    );
}

export default Login;